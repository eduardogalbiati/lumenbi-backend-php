...
class comprasQtdValoresDataMapper extends AbstractDataMapper
{
    protected $db;
    ...
    public function insertComprasMensal(array $compras)
    {
        ...
        $this->dropTable("Auper.dbo.comprasQtdValores");
        foreach ($compras as $line) {
            $this->insertTableArray('Auper.dbo.comprasQtdValores', $line);
        }
    ...
    protected function getAllData()
    {
        $stmt = $this->db->prepare("SELECT ano, mes, dia, valor as total
            FROM Auper.dbo.comprasQtdValores ORDER BY ano desc, mes desc, dia desc");
        $stmt->execute();
    ...
    protected function getAnualData()
    {
        $stmt = $this->db->prepare("SELECT ano, SUM(valor) as total
            FROM Auper.dbo.comprasQtdValores GROUP BY ano  ORDER BY ano asc");
        $stmt->execute();
        return $stmt->fetchAll();
    }
    ...
    protected function getMensalData($ano)
    {
        $stmt = $this->db->prepare("SELECT mes, SUM(valor) as total
            FROM Auper.dbo.comprasQtdValores WHERE ano=:ano  GROUP BY ano, mes ORDER BY mes asc");
    ...
    protected function getDiarioData($mes, $ano)
    {
        $stmt = $this->db->prepare("SELECT dia, SUM(valor) as total
            FROM Auper.dbo.comprasQtdValores WHERE mes=:mes AND ano=:ano GROUP BY mes, dia");
    ...
    public function loadPeriodo($dataInicio, $dataFim, $view)
    {
        if($view == 'Anual'){
            $stmt = $this->db->prepare("SELECT ano, SUM(valor) as total
                FROM Auper.dbo.comprasQtdValores WHERE datahora >= :dataInicio AND datahora <= :dataFim GROUP BY ano");
        }
        if($view == 'Mensal'){
            $stmt = $this->db->prepare("SELECT ano, mes, SUM(valor) as total
                FROM Auper.dbo.comprasQtdValores WHERE datahora >= :dataInicio AND datahora <= :dataFim GROUP BY ano, mes");
        
        }
        if($view == 'Diario'){
            $stmt = $this->db->prepare("SELECT ano, mes, dia, SUM(valor) as total
                FROM Auper.dbo.comprasQtdValores WHERE datahora >= :dataInicio AND datahora <= :dataFim GROUP BY ano, mes, dia");
        }
